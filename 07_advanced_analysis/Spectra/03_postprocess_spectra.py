...
def get_vocab_from_adata(adata, geneset_annotations):
    ...
    geneset_genes = []
    for i in geneset_annotations:
        for k in geneset_annotations[i]:
            geneset_genes += geneset_annotations[i][k]
    ...
    #vocab = pd.DataFrame(list(adata.var_names[adata.var['spectra_vocab']]), columns=[gene_key])
    return(vocab)
...
def annotate_factors(adata, geneset_anno, factor_names):
    ...
    # look at the overlap of each factors markers and all of the input gene sets in the spectra dictionary
    for i, fac in enumerate(factor_names):
        fac_genes = [str(x) for x in adata.uns['SPECTRA_markers'][i,:]] # top 50 genes for factor i
        for celltype, gene_sets in geneset_anno.items():
            for g in gene_sets:
...
def main():
    ...
    vocab = get_vocab_from_adata(adata, geneset_annotations)
    ...
    # TASK 2: ASSIGN FACTORS TO GENE PROGRAMS
    factor_geneset_anno = annotate_factors(adata, geneset_annotations, list(gene_weights.index))
    ...
    #adata.write(results_dir + run_name + '_adata.h5ad')
